...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$items = @('dist\hikvision-hours-processor.exe','dist\date.xlsx','dist\info.xlsx','dist\feriados_nacionales_argentina_2026.xlsx','dist\logo.png','dist\MANUAL_USUARIO_APP_DATE.xlsx.docx') | Where-Object { Test-Path -LiteralPath $_ }; Compress-Archive -Path $items -DestinationPath 'dist\hikvision-hours-processor-portable.zip' -Force"
